...
# quick guard
stopifnot(all(c("Party","LegisAny","IRA","CHIPS","CARES","PPP_HCE","COVID","TCJA","is_debt") %in% names(tw)))
...
overall_list <- list(
  ...
  IRA      = cond_deficit(tw, "IRA")      %>% mutate(Window = "IRA Window"),
  CHIPS    = cond_deficit(tw, "CHIPS")    %>% mutate(Window = "CHIPS Window"),
  ...
  TCJA     = cond_deficit(tw, "TCJA")     %>% mutate(Window = "TCJA Window")    # <-- NEW ROW
)
...
by_party <- bind_rows(
  ...
  tw %>% group_by(Party) %>% cond_deficit("IRA")      %>% mutate(Window = "IRA Window"),
  tw %>% group_by(Party) %>% cond_deficit("CHIPS")    %>% mutate(Window = "CHIPS Window"),
  ...
  tw %>% group_by(Party) %>% cond_deficit("TCJA")     %>% mutate(Window = "TCJA Window")  # <-- NEW ROW
) %>%
